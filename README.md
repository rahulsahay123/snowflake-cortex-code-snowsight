# ❄️ Snowflake Cortex Code — Snowsight Demo

> *What if you could explore, profile and build a full data pipeline just by describing what you want — in plain english?*
> *That's exactly what this demo shows.*

---

## 📽️ Demo Video

🎬 **[Watch the Demo on LinkedIn](#)**
*(Video coming soon)*

---

## 🎯 What is this?

This repo is a **prompt-first guide** to Snowflake Cortex Code in Snowsight.

Every SQL script in this repo was generated by typing a plain english prompt into the Cortex Code panel — no manual SQL writing, no schema memorization, no documentation needed.

The focus is on **how to write the right prompt** — not on the SQL output itself.

---

## 👤 Persona

**Day 1 Data Engineer.** New team. 8 tables. No documentation. No ERD. Boss wants a pipeline by EOD.

This demo shows how Cortex Code becomes your co-pilot from the first prompt to a running dbt pipeline.

---

## 📦 Dataset

| Property | Value |
|---|---|
| Database | SNOWFLAKE_SAMPLE_DATA |
| Schema | TPCH_SF1 |
| Domain | Supply Chain |
| Tables | 8 |
| Total Rows | ~8.6 Million |

---

## 🗂️ Repo Structure

```
snowflake-cortex-code-snowsight/
├── README.md
├── prompts/                           ← START HERE — all prompts
│   ├── 00_context_setting.md
│   ├── 01_table_discovery.md
│   ├── 02_object_inventory.md
│   ├── 03_documentation_check.md
│   ├── 04_data_dictionary.md
│   ├── 05_data_profiling.md
│   ├── 06_business_analysis.md
│   └── 07_dbt_pipeline.md
├── sql/
│   └── cortex_code_demo_complete.sql  ← generated output for reference only
└── dbt/testdbt/
    ├── dbt_project.yml
    ├── profiles.yml
    └── models/
        ├── staging/
        │   ├── sources.yml
        │   └── stg_orders.sql
        ├── intermediate/
        │   └── int_orders_enriched.sql
        └── mart/
            └── mart_revenue_by_segment.sql
```

> 💡 **The `/prompts` folder is the heart of this repo.** Each file contains the exact prompt, what to expect and key talking points. The `/sql` folder is the generated output — for reference only.

---

## 🚀 Demo Flow — Prompt by Prompt

### 🟦 Block 0 — Set Context
**One prompt. Four commands. Zero ambiguity.**

> *"Before I begin, set my working context: role = ACCOUNTADMIN, warehouse = COMPUTE_WH, database = SNOWFLAKE_SAMPLE_DATA, schema = TPCH_SF1. Confirm each step."*

---

### 🟦 Block 1 — Discover Tables
**Ask Cortex what exists — without opening a single table.**

> *"How many tables are in the TPCH_SF1 schema? Show me the record count for each table in a single query, ordered by row count descending."*

✅ Cortex chose INFORMATION_SCHEMA over full table scans — zero compute cost.

---

### 🟦 Block 2 — Object Inventory
**Check if anyone left anything behind.**

> *"In the current schema TPCH_SF1, show me the count of user defined views, functions and stored procedures grouped by object type in a single query."*

✅ All zeros. Clean slate confirmed.

---

### 🟦 Block 3 — Documentation Check
**Confirm there is nothing to read.**

> *"Check if any tables or columns in TPCH_SF1 have comments or descriptions defined. Show table name, column name and the comment in a single query."*

✅ Zero rows. No documentation exists. Now watch what happens next.

---

### 🟦 Block 4 — Data Dictionary
**From zero docs to a full data dictionary — in one prompt.**

> *"Generate a complete data dictionary for all tables in TPCH_SF1 schema showing table name, column name, data type, nullable flag, ordinal position and a business description for each column. Use INFORMATION_SCHEMA for structure and your knowledge of the TPCH data model to generate meaningful business descriptions. Cover all 8 tables. Order by table name and ordinal position."*

✅ 61 columns across 8 tables with business descriptions — generated from column naming conventions alone.

> 💡 **Key Insight:** Cortex used column prefixes like C_, O_, L_ to understand business context. Your naming convention is the first layer of AI readiness in your data platform.

---

### 🟦 Block 5 — Data Profiling
**Profile 1.5 million rows — with one prompt.**

> *"Profile the ORDERS table in TPCH_SF1 schema using a single query. For each column show: column name, data type, total row count, null count, null percentage, distinct value count. For numeric columns additionally show min, max and average. For date columns show earliest and latest date. Use INFORMATION_SCHEMA to dynamically pick up column metadata — do not hardcode column names. Round all decimals to 2 places."*

✅ Full column profile of 1.5M rows. No manual SQL. No hardcoded column names.

---

### 🟦 Block 6 — Business Analysis
**Describe the business question. Cortex figures out the joins.**

No column names. No table aliases. Just business intent.

| Prompt Intent | Tables Joined | Result |
|---|---|---|
| Net revenue by market segment and region | 5 tables | 25 rows in 2.7s |
| Suppliers with most delayed shipments | 3 tables | Top 10 in 1.5s |
| Part supply margin vs retail price | 2 tables | Top 10 by margin % |

> 💡 **Key Insight:** Cortex read the schema metadata, identified correct tables and columns, applied the right formulas — all from plain english. No column names were supplied.

---

### 🟦 Block 7 — dbt Pipeline
**Three prompts. Three models. One working pipeline.**

| Prompt | Output |
|---|---|
| Create target database | DBT_TPCH_ANALYTICS database |
| Configure profiles.yml | Snowflake oauth connection |
| Configure dbt_project.yml | 3 layer materialization strategy |
| Generate sources.yml | Source definitions with descriptions |
| Create stg_orders | Staging view — clean and cast |
| Create int_orders_enriched | Intermediate view — enriched with geography |
| Create mart_revenue_by_segment | Mart table — analytics ready |
| Run all models | PASS=3 WARN=0 ERROR=0 |

> 💡 **Key Insight:** Cortex understood dbt conventions — ref(), source(), materialization strategies — without being taught them. Production-ready pipeline from plain english prompts.

---

## 💡 Prompt Writing Best Practices

**1. Set context first — always**
Role, warehouse, database, schema before any query.

**2. Use business language not technical language**
Describe what you want, not how to get it. Cortex figures out the columns.

**3. Add cost efficiency instructions**
"Use INFORMATION_SCHEMA", "single query", "do not hardcode column names" — nudges Cortex toward smarter, cheaper solutions.

**4. Anticipate Cortex limitations in your prompt**
"Use dbt run not dbt compile" — being explicit avoids common mistakes.

**5. One task per prompt**
Scoped prompts produce better output than multi-task prompts.

---

## 📊 ROI Summary

| Task | Without Cortex Code | With Cortex Code |
|---|---|---|
| Schema understanding | 2-3 hours | 30 seconds |
| Data dictionary | Days | 2 minutes |
| Business queries | Hours | Seconds |
| dbt pipeline setup | Half a day | 15-20 minutes |
| **Total** | **~5 hours** | **~20-30 minutes** |

---

## 🛠️ Prerequisites

- Snowflake account with Cortex Code enabled
- Cross-region inference enabled
- ACCOUNTADMIN role or equivalent
- Access to SNOWFLAKE_SAMPLE_DATA database
- dbt project in Snowsight (for Block 7)

---

## 📖 Medium Article

📄 **[Read the full walkthrough on Medium](#)**
*(Coming soon)*

---

## 🤝 Connect

**Author:** Rahul Sahay
**LinkedIn:** [linkedin.com/in/rahulsahay](#)
**GitHub:** [github.com/rahulsahay123](https://github.com/rahulsahay123)

---

⭐ **If this helped you — give the repo a star and share it with your data engineering network.**
